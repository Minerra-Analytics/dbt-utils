...
with default_data as (
    ...
    select
        ...
        cast({{ dbt.string_literal(dwa.get_single_value(false_statement, '2022-01-01')) }} as {{ dbt.type_timestamp() }}) as date_actual,

        1.23456 as float_expected,
        {{ dwa.get_single_value(false_statement, 1.23456) }} as float_actual,

        123456 as int_expected,
        {{ dwa.get_single_value(false_statement, 123456) }} as int_actual,

        cast({{ dbt.string_literal('fallback') }} as {{ dbt.type_string() }}) as string_expected,
        cast({{ dbt.string_literal(dwa.get_single_value(false_statement, 'fallback')) }} as {{ dbt.type_string() }}) as string_actual
