...
with default_data as (
    ...
    select
        ...
        cast({{ dbt.string_literal(dwa.get_single_value(date_statement)) }} as {{ dbt.type_timestamp() }}) as date_actual,

        float_value as float_expected,
        {{ dwa.get_single_value(float_statement) }} as float_actual,

        int_value as int_expected,
        {{ dwa.get_single_value(int_statement) }} as int_actual,

        string_value as string_expected,
        cast({{ dbt.string_literal(dwa.get_single_value(string_statement)) }} as {{ dbt.type_string() }}) as string_actual
